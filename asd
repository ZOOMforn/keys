...
local requestsDisabled = true --getgenv and getgenv().DISABLE_ZOOM_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local ZOOMFolder = "ZOOM"
local ConfigurationFolder = ZOOMFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		ZOOMOpen = {Type = 'bind', Value = 'K', Name = 'ZOOM Keybind'},
		-- buildwarnings
		-- ZOOMprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.ZOOMOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(ZOOMFolder) then
				if isfile and isfile(ZOOMFolder..'/settings'..ConfigurationExtension) then
					file = readfile(ZOOMFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('ZOOM had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("ZOOM", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify ZOOM developers at sirius.menu/discord.")
				end
			end)
...
local ZOOM = useStudio and script.Parent:FindFirstChild('ZOOM') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local ZOOMDestroyed = false -- True when ZOOMLibrary:Destroy() is called

repeat
	if ZOOM:FindFirstChild('Build') and ZOOM.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('ZOOM | Build Mismatch')
		print('ZOOM may encounter issues as you are running an incompatible interface version ('.. ((ZOOM:FindFirstChild('Build') and ZOOM.Build.Value) or 'No Build') ..').\n\nThis version of ZOOM is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, ZOOM = ZOOM, useStudio and script.Parent:FindFirstChild('ZOOM') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(ZOOM)
	ZOOM.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	ZOOM.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ZOOM.Name and Interface ~= ZOOM then
			Interface.Enabled = false
			Interface.Name = "ZOOM-Old"
		end
	end
elseif not useStudio then
...
if ZOOM.AbsoluteSize.X < minSize.X and ZOOM.AbsoluteSize.Y < minSize.Y then
...
local Main = ZOOM.Main
local MPrompt = ZOOM:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = ZOOM:FindFirstChild('Drag')
...
ZOOM.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/ZOOM/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = ZOOM.Notifications

local SelectedTheme = ZOOMLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = ZOOMLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	ZOOM.Main.BackgroundColor3 = SelectedTheme.Background
	ZOOM.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ZOOM.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ZOOM.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	ZOOM.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ZOOM.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ZOOM.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		ZOOM.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		ZOOM.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(ZOOM:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("ZOOM | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("ZOOM | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('ZOOM had an issue decoding the configuration file, please try delete the file and reopen ZOOM.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(ZOOMLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("ZOOM | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--ZOOMLibrary:Notify({Title = "ZOOM Flags", Content = "ZOOM was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(ZOOMLibrary.Flags) do
...
function ZOOMLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('ZOOM | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			ZOOMLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show ZOOM'.", Duration = 7, Image = 4400697855})
		else
			ZOOMLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "ZOOMOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(ZOOMFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('ZOOM Settings', 0, true)

	if TabList['ZOOM Settings'] then
		TabList['ZOOM Settings'].LayoutOrder = 1000
	end

	if Elements['ZOOM Settings'] then
		Elements['ZOOM Settings'].LayoutOrder = 1000
	end
...
function ZOOMLibrary:CreateWindow(Settings)
	if ZOOM:FindFirstChild('Loading') then
		if getgenv and not getgenv().ZOOMCached then
			ZOOM.Enabled = true
			ZOOM.Loading.Visible = true

			task.wait(1.4)
			ZOOM.Loading.Visible = false
		end
	end

	if getgenv then getgenv().ZOOMCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				ZOOMLibrary:Notify({Title = 'Build Mismatch', Content = 'ZOOM may encounter issues as you are running an incompatible interface version ('.. ((ZOOM:FindFirstChild('Build') and ZOOM.Build.Value) or 'No Build') ..').\n\nThis version of ZOOM is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ZOOM"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "ZOOM Interface Suite" then
		LoadingFrame.Version.Text = "ZOOM UI"
	end
	...
	if not Settings.DisableZOOMPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(ZOOMFolder.."/Discord Invites") then
			makefolder(ZOOMFolder.."/Discord Invites")
		end

		if isfile and not isfile(ZOOMFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ZOOMFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ZOOM RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(ZOOMFolder.."/Key System") then
			makefolder(ZOOMFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("ZOOM | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZOOM specific development.')
				end
			end
		end
		...
		if isfile and isfile(ZOOMFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ZOOMFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ZOOMFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ZOOMLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called ZOOMLibrary:Destroy()
				if ZOOMDestroyed then
					return
				end
				if not Success then
					...
					print("ZOOM | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZOOM specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ZOOMLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ZOOMLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			ZOOM.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ZOOM | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZOOM specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					ZOOMLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("ZOOM | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with ZOOM specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ZOOMLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ZOOM | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with ZOOM specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ZOOMLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= ZOOMLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ZOOM | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with ZOOM specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						ZOOMLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ZOOM | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with ZOOM specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ZOOMLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			ZOOMLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			ZOOMLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('ZOOM had an issue creating settings.') end
...
function ZOOMLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function ZOOMLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function ZOOMLibrary:Destroy()
	ZOOMDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['ZOOM Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "ZOOMOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function ZOOMLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				ZOOMLibrary:Notify({Title = "ZOOM Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			ZOOMLibrary:Notify({Title = "ZOOM Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('ZOOM Configurations Error | '..tostring(result))
			ZOOMLibrary:Notify({Title = "ZOOM Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = ZOOMLibrary:CreateWindow({
		Name = "ZOOM Example Window",
		LoadingTitle = "ZOOM Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using ZOOM may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like ZOOM to get the key from
	...
	--ZOOMLibrary:Notify({Title = "ZOOM Interface", Content = "Welcome to ZOOM. These - are the brand new notification design for ZOOM, with custom sizing and ZOOM calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(ZOOMLibrary.Theme) do
